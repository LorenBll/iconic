...
import ObsidianUtils from './ObsidianUtils.js';
...
class Strings {
	 ...
	 */
	static async localize(): Promise<void> {
		let localizedStrings: { default: Record<string, unknown> };
		switch (getLanguage()) {
		...
		this.localizeDictionary(this.dictionary, localizedStrings.default);
	}
	 ...
	 */
	private static localizeDictionary(currentStrings: Record<string, unknown>, localizedStrings: Record<string, unknown>): void {
		for (const [key, localizedValue] of Object.entries(localizedStrings)) {
			const currentValue = currentStrings[key];
			if (ObsidianUtils.isObject(currentValue) && ObsidianUtils.isObject(localizedValue)) {
				this.localizeDictionary(currentValue, localizedValue);
			} else if (typeof currentValue === 'string' && typeof localizedValue === 'string') {
				currentStrings[key] = localizedValue;
			}
		}
	}
}

export default Strings.dictionary;
